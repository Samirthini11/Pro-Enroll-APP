...
class IFreeCookieSolver {
  ...
  Future<bool> solveFromResponse(http.Response response) async {
    try {
      ...
      if (match == null) {
        final aMatch =
            RegExp(r'var a\s*=\s*toNumbers\("([a-f0-9]+)"\)').firstMatch(body);
        final bMatch = RegExp(r'b\s*=\s*toNumbers\("([a-f0-9]+)"\)')
            .firstMatch(body);
        final cMatch = RegExp(r'c\s*=\s*toNumbers\("([a-f0-9]+)"\)')
            .firstMatch(body);
        if (aMatch == null || bMatch == null || cMatch == null) return false;
        _cookie = _decrypt(aMatch.group(1)!, bMatch.group(1)!, cMatch.group(1)!);
      } else {
